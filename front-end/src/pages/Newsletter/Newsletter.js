...
const Newsletter = () => {
  ...
  const fetchSubscribers = async () => {
    try {
      const response = await axios.get('https://ezemmuoblockchain-5.onrender.com/api/all_newsletters');
  ...
  const delSub = async (id) => {
    try {
      await axios.delete(`https://ezemmuoblockchain-5.onrender.com/api/newsletter/${id}`);
